...
/* Border Beam Panel — from Motiq (https://motiq.dev/components/border-beam-panel).
   ...
   Two local changes vs. upstream:

   1. The `MOTIQ_TOKENS` <style> block the vendor file renders alongside every
      instance has been removed. app/globals.css defines the same --motiq-*
      names in an `@layer motiq` and re-points them at the ShieldGate palette,
      so injecting them per-instance would be dead weight on a page that
      renders a dozen panels. The bridge lives in one place instead.

   2. `children` is wrapped in `.mk-beam-content`. The glow is a blurred copy of
      the ring at `z-index: -1`, but the panel sets `isolation: isolate` — and
      inside a stacking context a negative-z child paints *above* that
      element's own background, not behind it. On the vendor's near-black demo
      surface that reads as cast light; on ShieldGate's light theme it is a
      coloured smudge across the middle of the card. The wrapper re-establishes
      an opaque surface above the glow, so only the part of the glow that
      spills outside the panel survives — which is the part that was ever
      meant to be seen. */
...
.${cls} .mk-beam-content {
  ...
  z-index: 1;
  border-radius: ${radius}px;
  /* inherit, rather than a fixed token, so the wrapper always matches
     whatever surface the consumer put on the panel itself. */
  background: inherit;
}
.${cls} .mk-beam-ring {
  z-index: 2; /* above the content surface — the ring IS the border */
...
@media (forced-colors: active) {
  ...
  return (
    ...
    >
      ...
      <div className="mk-beam-content">{children}</div>
    </div>
  );
}
